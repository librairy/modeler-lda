 ...
 * @author cbadenes
 */
public class LDASimilarityTaskBackup implements Runnable {

    private static final Logger LOG = LoggerFactory.getLogger(LDASimilarityTaskBackup.class);
    ...
    public LDASimilarityTaskBackup(String domainUri, ModelingHelper modelingHelper) throws DataNotFound {
        this.domainUri = domainUri;
        this.helper = modelingHelper;
        this.topics = helper.getCounterDao().getValue(domainUri, Resource.Type.TOPIC.route());
    }
...
//                        .option("spark.sql.autoBroadcastJoinThreshold","-1")
                    ...
                    Integer maxPointsPerCluster = 1000; //2000
    ...
    private void similaritiesFromCentroids(ComputingContext context, JavaRDD<Tuple2<String,Vector>> documents, int maxSize, int maxIterations, double epsilon, AtomicInteger counter){
        ...
        for (int cid = 0; cid < centroids.length ; cid ++){
            ...
            if (numPoints <= maxSize){
                ...
                LOG.info("similarities in cluster " + index + " calculated with " + numPoints + " documents in " + ChronoUnit.MINUTES.between(startProc,endProc) + "min " + (ChronoUnit.SECONDS.between(startProc,endProc)%60) + "secs");
                // Increment counter
...
//        simRows.unpersist();
        ...
        LOG.info("saving documents from sector "+centroid.getId());
...
//        helper.getSimilarityService().saveSubGraphToFileSystem(context, nodesFrame,URIGenerator.retrieveId(domainUri), "nodes", String.valueOf(centroid.getId()));
    ...
    private List<Double> calculateCentroid(RDD<Vector> points){
        points.persist(helper.getCacheModeHelper().getLevel());
        points.take(1);
